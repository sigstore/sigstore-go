...
func NewTlogEntry(tle *v1.TransparencyLogEntry) (*Entry, error) {
	...
	// shallow copy the TLE
	tleCopy := v1.TransparencyLogEntry{
		LogIndex:          tle.LogIndex,
		LogId:             tle.LogId,
		KindVersion:       tle.KindVersion,
		IntegratedTime:    tle.IntegratedTime,
		InclusionPromise:  tle.InclusionPromise,
		CanonicalizedBody: tle.CanonicalizedBody,
	}
	if tle.InclusionProof != nil {
		inclusionProof := v1.InclusionProof{
			LogIndex:   tle.InclusionProof.LogIndex,
			TreeSize:   tle.InclusionProof.TreeSize,
		...
		hex.Encode(rootHash, tle.InclusionProof.RootHash)
		inclusionProof.RootHash = rootHash
		tleCopy.InclusionProof = &inclusionProof
	}
	entry.tle = &tleCopy
...
func ValidateEntry(entry *Entry) error {
	...
	if entry.rekorV2Entry != nil { // nolint:revive,staticcheck // FIXME
		// TODO
	}
...
func (entry *Entry) Signature() []byte {
	...
	if entry.rekorV2Entry != nil {
		return entry.rekorV2Entry.GetSpec().GetHashedRekordV002().GetSignature().GetContent() // TODO: dsse
	}
...
func (entry *Entry) PublicKey() any {
	...
	if entry.rekorV1Entry != nil {
		...
		verifier := entry.rekorV2Entry.GetSpec().GetHashedRekordV002().GetSignature().GetVerifier() // TODO: dsse
